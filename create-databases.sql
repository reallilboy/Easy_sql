--The SQL DROP DATABASE IF EXISTS statement includes a condition to check whether the database exists before trying to delete it.
--با استفاده از دستور DROP و IF EXISTS می توانید از حذف ناخواسته اشیاء خود در دیتابیس جلوگیری به عل آورد و به همین منوال برای حذف سایر اشیاء یک دیتابیس هم می توانید استفاده کنید.
DROP DATABASE IF EXISTS `sql_invoicing`;
*******************************************************************
--If the administrator creates your database for you when setting up your permissions, you can begin using it. Otherwise, you need to create it yourself
--با دستور create datbase یک دیتابیس ایجاد میکنیم
CREATE DATABASE `sql_invoicing`; 
--Creating a database does not select it for use; you must do that explicitly. To make menagerie the current database, use this statement:
--یا استفاده از دستور use یک دیتابیس برای استفاده انتخاب کرده.
USE `sql_invoicing`;

--The SET command is used with UPDATE to specify which columns and values that should be updated in a table.
--دستور Set در Sql به معنی قرار دادن یک مقدار در فیلد یا متغیر می‌باشد
SET NAMES utf8 ;
...
--از دستور INSERT INTO برای وارد کردن record جدید به جدول استفاده می شود 
...
--با استفاذه از این محدودیت اجازه نمیدهیم که فیلد ما مقدار Null بگیرد. 
 ...
 --Constraint برای تعیین قوانینی برای داده ها در یک جدول استفاده می شود
